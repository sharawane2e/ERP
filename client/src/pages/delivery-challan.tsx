...
import type { Project, Client, DeliveryChallan, DeliveryChallanItem, Branding } from "@shared/schema";
...
type GatePassType = "Delivery Challan";
...
export default function DeliveryChallanPage() {
  type SignatureField =
    ...
    | "plantHeadSignature";
  const params = useParams<{ id: string; deliveryChallanId?: string }>();
  const projectId = params.id;
  const deliveryChallanId = params.deliveryChallanId;
  ...
  const [gatePassData, setGatePassData] = useState({
    gatePassNumber: "",
    revision: "R-001",
    gatePassType: "Delivery Challan" as GatePassType,
    issueNo: "001",
    issueDate: "",
    revNo: "00",
    revDate: "",
  ...
  const buildDispatchSummary = useCallback(() => {
    ...
    if (gatePassData.showDispatchGatePass) {
      parts.push(`Delivery Challan: ${gatePassData.dispatchGatePass || "-"}`);
    }
  ...
  // Set defaults for New Delivery Challan
  useEffect(() => {
    if (project && client && !existingGatePass) {
      ...
      setGatePassData(prev => ({
        ...prev,
        gatePassNumber: `RNS/${shortName}/${docDate}/RNS-DC-002`,
  ...
  // Load existing Delivery Challan data
  useEffect(() => {
    if (existingGatePass) {
      ...
      const transportParts = legacyTransport.includes("-")
        ...
        : [];
      setGatePassData({
        gatePassNumber: existingGatePass.deliveryChallanNumber,
        revision: existingGatePass.revision,
        gatePassType: parsedContent.gatePassType || "Delivery Challan",
  ...
  const mapLineItemToApiPayload = (item: GatePassLineItem, idx: number) => {
    const description = item.materialDescription.trim() || item.partMark.trim() || "ITEM";
    const rawRate = String(item.materialSize ?? "").replace(/,/g, "").trim();
    const hasNumericRate = /^-?\d+(\.\d+)?$/.test(rawRate);

    return {
      serialNo: item.serialNo > 0 ? item.serialNo : idx + 1,
      description,
      hsnCode: item.partMark.trim() || undefined,
      quantity: toDecimalString(item.quantity),
      unit: item.asslyPartSl.trim() || "NOS",
      ratePerUnit: hasNumericRate ? toDecimalString(rawRate, "0") : "0",
      percentage: "0",
  ...
  const getPersistableLineItems = useCallback(() => {
    const hasMeaningfulValue = (value: string | number) => {
    ...
    return lineItems
      .slice(0, 5)
      .filter((item) =>
        hasMeaningfulValue(item.materialDescription) ||
        hasMeaningfulValue(item.materialSize) ||
        hasMeaningfulValue(item.partMark) ||
        hasMeaningfulValue(item.asslyPartSl) ||
        hasMeaningfulValue(item.approxValue) ||
  ...
  const duplicateGatePassMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || "Failed to duplicate delivery challan",
        variant: "destructive",
      });
    },
  });
  ...
  const exportToPDF = async (opts?: { save?: boolean; silent?: boolean; returnDataUri?: boolean }) => {
    ...
    try {
      ...
      const gatePassTitle = "DELIVERY CHALLAN";
      ...
      addSectionTitle("Delivery Challan Details", 11);
      addFourColumnClientTable(
        [
          ["Doc. No", gatePassData.gatePassNumber || "RNS/PROJECT/00-00-00/RNS-DC-002"],
          ["Date", issueDateText || "-"],
      ...
      const tableHeaders = ["SR. NO.", "MATERIAL DISCRIPTION", "MATERIAL SIZE", "ITEM CODE", "UNIT", "QUANTITY", "APPROX. VALUE"];
      const colWidths = [14, 40, 29, 25, 24, 20, 28];
  ...
  const submitSendEmail = async () => {
    ...
    try {
      ...
      const emailRes = await apiRequest("POST", `/api/delivery-challans/${existingGatePass.id}/send-email`, {
        to: emails,
  ...
  const submitSendWhatsApp = async () => {
    ...
    try {
      const base64 = generated.dataUri.split(",")[1] || "";
      await apiRequest("POST", `/api/delivery-challans/${existingGatePass.id}/send-whatsapp`, {
        to: phones,
  ...
  return (
    <LayoutShell user={user}>
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            {existingGatePass && (
              <Button
                ...
                className="bg-white/10 border-white/20 text-white hover:bg-white/20"
                data-testid="button-duplicate-delivery-challan"
              >
            ...
            <Button
              ...
              className="bg-white/10 border-white/20 text-white hover:bg-white/20"
              data-testid="button-save-delivery-challan"
            >
              ...
              <Button 
                ...
                onClick={() =>
                  setConfirmDialog({
                    ...
                    description: `Are you sure you want to delete this Delivery Challan (${existingGatePass.revision})?`,
                    onConfirm: () => deleteGatePassMutation.mutate(existingGatePass.id),
                  })
                }
                data-testid="button-delete-delivery-challan"
              >
        ...
        <Card>
          <CardHeader className="bg-slate-100">
            <CardTitle className="text-lg text-slate-700">Delivery Challan ITEMS</CardTitle>
          </CardHeader>
              ...
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="p-3 text-left text-sm font-medium text-slate-600 w-44">ITEM CODE</th>
                    <th className="p-3 text-left text-sm font-medium text-slate-600 w-28">UNIT</th>
      ...
      <Dialog open={versionsDialogOpen} onOpenChange={setVersionsDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Delivery Challan Versions</DialogTitle>
          </DialogHeader>
                  ...
                  {gatePassVersions && gatePassVersions.length > 1 && (
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        setConfirmDialog({
                          open: true,
                          title: "Delete Delivery Challan Version?",
      ...
      <ConfirmActionDialog
        ...
        confirmLabel="Delete"
        confirmTestId="confirm-delete-delivery-challan-action"
        cancelTestId="cancel-delete-delivery-challan-action"
      />
    </LayoutShell>
  );
}
